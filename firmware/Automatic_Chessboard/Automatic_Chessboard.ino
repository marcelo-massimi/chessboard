/**
 * @file Automatic_Chessboard.ino
 * @brief Sistema de control principal del tablero de ajedrez automatizado
 * @author Original: Greg06, Modificaciones: marcelo-massimi
 * @see https://www.instructables.com/Automated-Chessboard/
 * 
 * Implementación principal del sistema de control del tablero de ajedrez.
 * Este archivo gestiona la interacción entre los distintos componentes:
 * - Detección de piezas mediante sensores
 * - Control de motores y electroimán
 * - Interfaz con el motor de ajedrez
 * - Gestión de la interfaz de usuario
 * 
 * @note Componentes de hardware:
 *       - Arduino Mega (requerido por uso de memoria)
 *       - Sensores Hall lineales para detección de piezas
 *       - Matriz LED 8x8 (MAX7219) para visualización de estado
 *       - Puente H L298 para control bidireccional del electroimán
 *       - Sistema multiplexado de sensores
 * 
 * @note Configuración del sistema:
 *       - Polarización: Invertida entre piezas blancas y negras
 *       - Posición inicial: H7 (modificada de E7 original)
 *       - Modo de operación: Buscar "Simulide" para alternar entre 
 *         simulación y hardware real
 */
...
//
// V10_3_7 again comments in spanish
// Se agrega pantalla final cuando en HvsC un jugador gana por Jaque Mate
//
// V10_3_8 again comments in spanish
// Solucionado: En HvsC cuando la AI hace la ultima jugada para Jaque Mate, termina el juego
// antes que el Trolley la haga. Convierto game_mode en externa para que esa ultima jugada 
// solo la haga en HvsC
//
// Agregado: Game Over cuando paso el tiempo en HvsH y en HvsC solo para Blancas
// Se modifica Version de Global para poder acceder a GameOver
//
// Agregado: fix_up_position en Black Castle
// Se cambia de lugar delay en Read Sensor para buscar aumentar sensibilidad
// Verificar: en HvsH, si se llama a fix_up_position, pues no es necesario
//
// V11 July 2025
// Se cambia la forma de comparar los Sensores. 
// Ver el Test: Read_64_HallSensor_V02
// Por comodidad, se coloca la inicializacion de los Sensores en un archivo separado
// hall_setup.cpp llamado desde hall_setup.h
//
// V11_3 Fixed Problem after no_valid_move
// Problem: Sometimes the Electromagnet after a no_valid_move "forget" the position and go to other square
//
...
//-----------------------------------------------------------------------------
#include "include/global.h"        // Configuraciones globales
#include "include/Micro_Max.h"     // Motor de ajedrez

//-----------------------------------------------------------------------------
// Configuración de dispositivos
//-----------------------------------------------------------------------------
// Display LCD I2C
LiquidCrystal_I2C lcd(0x27, 16, 2);   // Dirección 0x27, 16x2 caracteres

// Matriz LED MAX7219
// DIN=51, CLK=52, CS=53, 1 matriz
...
void setup() {
    ...
    Serial.begin(9600);
    Serial.println("Automatic Chessboard v11.3 - Inicializando...");

    //-------------------------------------------------------------------------
    // Configuración de pines
    //-------------------------------------------------------------------------
    
    // Sistema de electroimán
    pinMode(IN1, OUTPUT);          // Control dirección 1
    pinMode(IN2, OUTPUT);          // Control dirección 2
    pinMode(MAGNET, OUTPUT);       // Control PWM del electroimán

    // Sistema de motores
    pinMode(MOTOR_WHITE_STEP, OUTPUT);  // Paso motor blancas
    pinMode(MOTOR_WHITE_DIR, OUTPUT);   // Dirección motor blancas
    pinMode(MOTOR_BLACK_STEP, OUTPUT);  // Paso motor negras
    pinMode(MOTOR_BLACK_DIR, OUTPUT);   // Dirección motor negras

    // Sistema de sensores
    pinMode(Mux_Out, INPUT_PULLUP);     // Entrada multiplexada con pull-up
    ...
    lc.setIntensity(0, 4);       // Intensidad media (0-15)
    lc.clearDisplay(0);          // Limpiar pantalla

    // Inicialización de arrays de estado
    initializeSensorArrays();     // Inicializa arrays de sensores
    lcd.init();               // Inicializar LCD
    lcd.backlight();           // Encender retroiluminación
}

//-----------------------------------------------------------------------------
// Inicialización de arrays de sensores
//-----------------------------------------------------------------------------
void initializeSensorArrays() {
    // Inicializar todos los arrays relacionados con sensores
    for(int i = 0; i < 8; i++) {
        for(int j = 0; j < 8; j++) {
            hall_sensor_status[i][j] = 0;       // Estado actual
            hall_sensor_status_memory[i][j] = 0; // Estado anterior
            hall_value[i][j] = 0;               // Valores brutos
        }
    }
}
    ...
    case player_white:
      ...
      if (button(WHITE) == true) {  // White player end turn
        ...
        if (game_mode == HvsH) {
          ...
          else { // O sea jugada White no fue valida en HvsC (Causas: humano no dejo la pieza centrada o sacó otra pieza del centro, o hizo una jugada ilegal)
          lcd_display(); // Aqui programo la opcion set_the_board en el lcd_display() para este caso, para que el humano corrija las piezas
...
void electromagnet(boolean state) 
{
  ...
  {
    analogWrite(MAGNET, pwmMax); // Pruebo directo valor maximo y no subir gradualmente
...
*/
  ...
  Serial.println(displacement_Y);       //V9.2.6
     ...
     motor(F8_F1, SPEED_SLOW, 0.5);
      ...
      motor(A_H, SPEED_FAST, arrival_coord_Letter + 0.75); // v11_3 Put the white piece out of the board *** You need an extra square of space on your board !!!
...
void lcd_display() {
  ...
  switch (sequence) {
    case start_up:
      ...
      delay(300); // V11 reduce delay()
      lcd.setCursor(0, 0);
      lcd.print(" NO ONE PIECES  "); // V11 add the Number Version
      lcd.setCursor(0, 1);
...
void read_hall_values(HC4067 &mux) { // V10_3_8 cambio lugar el delay y lo reduzco a 1
  ...
  for (int j = 0; j < 16; j++) {
    ...
    Record_hall_measure_V2(); // Graba el valor del sensor activado
    column++;
    if (column > 7) {
      column = 0;
      row++;
...
void Record_hall_measure_V2() {  // V11 ***
  ...
  } else if (hallMeasure > hall_value_south_magnet[row][7-column]) {
...
void hall_display() {
 ...
 // *** hall status and Matrix
  Serial.println("*** hall_sensor_status ****");
  Serial.println("+ - - - - - - - - -+");
  for (int i = 7; i >=0; i--) {
    Serial.print(' ');
    Serial.print(i+1);
...
// ************ Funciones para Setear los Sensores
...
void read_hall_values_setup(HC4067 &mux) { // V10_3_8 cambio lugar el delay y lo reduzco a 1
  ...
  for (int j = 0; j < 16; j++) {
    ...
    hall_value_no_magnet[row][7-column] = hallMeasure; // Graba el valor del sensor activado. En Esta Matriz se guarda el valor de cada sensor sin Iman cerca. 
    hall_value_south_magnet[row][7-column] = hallMeasure + hall_compare; // Hall49e To92 is South Faced, biger value when South pole detected
    hall_value_north_magnet[row][7-column] = hallMeasure - hall_compare;  // Hall49e To92 is South Faced, lower value when North pole detected
...
void read_hall_values_setup_0(HC4067 &mux) { // V10_3_8 cambio lugar el delay y lo reduzco a 1
  ...
  for (int j = 0; j < 16; j++) {
    ...
    hall_value_no_magnet_memory[row][7-column] = hallMeasure; // Graba el valor del sensor activado. En Esta Matriz se guarda el valor de cada sensor sin Iman cerca. 
...
void Read_Sensor_Setup_0 (HC4067 &mux) 
{
...
void ordenarYMostrarMatriz(int matriz[8][8]) {
  
  // Paso 1: Copiar los valores a un arreglo unidimensional
  int valores[8 * 8];
  int index = 0;
  for (int i = 0; i < 8; i++) {
    for (int j = 0; j < 8; j++) {
  ...
  // Paso 2: Ordenar el arreglo de mayor a menor (usando bubble sort)
  for (int i = 0; i < 8 * 8 - 1; i++) {
    for (int j = 0; j < 8 * 8 - i - 1; j++) {
  ...
  // Paso 3: Imprimir los valores separados por comas
  for (int i = 0; i < 8 * 8; i++) {
