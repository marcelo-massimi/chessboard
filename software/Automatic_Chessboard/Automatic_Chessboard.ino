...
#include "global.h"  // July 2025
// #include "hall_setup_V1.h" // V11 New file added for easy folow the code on the setup sensors. You can manualy run the Read_64_HallSensor_V02 and add the values, but I preffer to add this file on this version
#include "Micro_Max.h" // V10.3.5 make b[] global for can acces it from here. b[] is the place where micromax save the board
#include <Wire.h>
...
// Matrix Led
  ...
  lc.clearDisplay(0);     // Turn off leds
    ...
    // Initialize the hall sensor memory array to 0 in all values // V10.3.8
    // hall_sensor_status_memory[8][8] = 0; // esto solo pone el valor en [8][8], no en cada lugar de la matriz
    // Initialize the hall sensor array to 0 in all vaues       // V10.3.8
    //hall_sensor_status[8][8] = 0;
    // Initialize the hall sensor array to 0 in all values       // V10.3.8
    //hall_value[8][8] = 0;

  // Initialize Matrix Values V10_8_3

  for (int v = 0; v < 8; v++) {
    for (int b = 0; b < 8; b++) {
      hall_sensor_status_memory[v][b] = 0;
